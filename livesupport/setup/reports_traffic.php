...
<!--
	...
	function show_div( thediv )
	{
		if ( thediv == "foot_main" )
		{
			$('#foot_settings').hide() ;
			$('#foot_main').show() ;
			$('#menu_foot_main').removeClass('op_submenu').addClass('op_submenu_focus') ;
			$('#menu_foot_settings').removeClass('op_submenu_focus').addClass('op_submenu') ;
		}
		else
		{
			$('#foot_main').hide() ;
			$('#foot_settings').show() ;
			$('#menu_foot_main').removeClass('op_submenu_focus').addClass('op_submenu') ;
			$('#menu_foot_settings').removeClass('op_submenu').addClass('op_submenu_focus') ;
		}
	}
	...
	function confirm_change( theflag )
	{
		...
		{
			if ( confirm( "Switch footprint logging "+theflag+"?" ) )
			{
				$.ajax({
					type: "POST",
					url: "../ajax/setup_actions.php",
					data: "ses=<?php echo $ses ?>&action=update_foot_log&value="+theflag+"&"+unixtime(),
				...
				$('#foot_log_'+global_foot_log).attr('checked', true) ;
		}
	}
...
<?php include_once( "./inc_header.php" ) ?>
		...
		<div class="op_submenu_wrapper">
			...
			<div class="op_submenu" onClick="show_div('foot_settings')" id="menu_foot_settings">Settings</div>
		...
		<div id="foot_main">
			...
			<div id="overview_day_chart" style="margin-top: 50px;">
				...
				<div id="overview_data_container">
					...
					<tr>
						<td><div style="max-height: 350px; overflow: auto;">
		...
		<div id="foot_settings" style="display: none; margin-top: 25px;">
			<div>
				...
				<ul style="margin-top: 10px;">
					...
					<li> <b>Off</b> does not affect the real-time traffic monitor or refer URL tracking.
				</ul>
			</div>

			<div style="margin-top: 25px;">
				<div class="li_op round"><input type="radio" name="foot_log" id="foot_log_on" value="on" onClick="confirm_change('on')" <?php echo $footprint_on ?>> Footprint Logging On</div>
				<div class="li_op round"><input type="radio" name="foot_log" id="foot_log_off" value="off" onClick="confirm_change('off')" <?php echo $footprint_off ?>> Footprint Logging Off</div>
				<div style="clear: both;"></div>
			</div>
		</div>
