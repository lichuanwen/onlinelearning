...
?>
    ...
    <div align="center" style="max-height: float;">
...
?>                  
    ...
    <div style="max-height: float;" class="stage" id="exception">  <!-- Stage {start} -->
          <!-- Page Content STARTS here -->
          
          
<form style="text-align:left;" id="proctor" name="proctor" method="post" action="process_proctors.php" onsubmit="return validate();">
  <fieldset>
...
$query = "SELECT CONCAT(CName,' ', CNum) AS 'CCode' FROM `courses` ORDER BY `CCode` ASC";
 $results = sendQuery($query);
	...
	if ($results){
            printf('<select name="courseCode" id="dd-courses" title="Select your online course">');
